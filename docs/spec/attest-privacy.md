...
| `size_bytes` | not personal | Integer, `0 ≤ n ≤ 2^53 − 1`; a property of a file. |
| `sha256` | potentially personal, user-controlled | Lowercase-hex SHA-256 of the artifact. See the per-buyer-build note below. |

**Per-buyer builds.** These rows describe files, not people — with one exception that the format permits and no `verifier` can detect. Where an `issuer` serves watermarked or otherwise personalized artifacts, the `filename` and the `sha256` of a buyer's own copy are unique to that buyer, signed into the payload, and identical in every copy of the receipt; they then function as a stable correlator joining that receipt to that buyer's downloaded file and to any other document quoting the same hash. Both fields are classified for that case, not for the common one. A hash of a shared release build carries nothing about anyone. Separately, artifact hashes identify content **authorized** under the issuer's mirror policy and MUST NOT be construed as a license to source matching-hash files from unauthorized hosts (v0.1 §5.4).
...
| `legal_text_sha256` | not personal | Lowercase-hex SHA-256 hash-binding the license text into the signed payload. The text itself travels in the export bundle (§2.14). |
...
| `mirror_policy_sha256` | not personal | Optional lowercase-hex SHA-256 hash-binding the mirror policy text, so the `issuer` cannot silently rewrite obligations post-issuance. |
...
| `eol_commitment_sha256` | not personal | Optional lowercase-hex SHA-256 or `null`, hash-binding an end-of-life commitment document. |
...
| `signatures[].sig` | not personal | Signature bytes over `JCS(payload)`: base64url, 64 decoded bytes for Ed25519, 3309 for ML-DSA-65. Deterministic given the key and the signed bytes; carries no additional information about anyone beyond what the payload already carries. |
...
| `delivery.issuer_manifest` | not personal | An optional embedded key-manifest snapshot (§2.8), consisting entirely of the issuer's public key material. A manifest that arrived this way is unauthenticated TOFU and MUST be reported as `trust: "unauthenticated_tofu"` (v0.1 §7.4); its privacy content is nil, since it is a snapshot of the same document an `issuer` SHOULD publish at its own well-known URL (v0.1 §7.1). |

### 2.8 Key manifests (v0.1 §7.1; v0.2 §2.3)

A key manifest is a public document, published by an `issuer` at a well-known URL, and it contains no buyer-derived value of any kind.
...
| `manifest_signature.kid` / `.sig` | not personal | Ed25519 over `JCS(manifest)` with this member removed. |
| `manifest_signature.sig_ml_dsa_65` | not personal | ML-DSA-65 leg, REQUIRED iff the signing key entry carries `pub_ml_dsa_65`, absent otherwise; AND-verified fail-closed in both directions (v0.2 §2.3). |

### 2.9 Artifact manifests (v0.1 §7.2; v0.2 §13)

An artifact manifest is series-level product state, shared by every buyer of that series, and carries no buyer-derived value.
...
| `manifest_signature` (`kid`, `sig`, optional `sig_ml_dsa_65`) | not personal | Same discipline as a key manifest, extended to the hybrid AND-rule by v0.2 §13. |

### 2.10 Revocation records (v0.1 §12; v0.2 §13)

A revocation record names a receipt. It carries no identifier, no commitment, no salt, and no reference to a `buyer` of any kind.
...
| `signature` (`kid`, `sig`, optional `sig_ml_dsa_65`) | not personal | Ed25519 over `JCS(record)` with this member removed, extended to the hybrid AND-rule by v0.2 §13. |
...
Log entries are **content-free by construction**. Exactly two versioned types are defined, each with exactly the required member set and no more; unknown members are rejected outright rather than silently tolerated; and every member is a domain name, a version integer, or a lowercase-hex hash. There is nowhere in an entry for personal data to live, by the same closure that leaves nowhere for attacker-chosen content to live (TM-51). No payload, no field of a payload, and no fragment of one is ever admitted to a log.
...
| Field | Classification | Notes |
| --- | --- | --- |
| `origin` | not personal | First header line; non-empty printable ASCII (`0x20`–`0x7e`), naming the log and pinned in the verifier's own configuration. |
...
| signature lines (`name`, key-hash, Ed25519 leg, ML-DSA-65 leg) | not personal | `— <name> <base64(key-hash \|\| signature)>`; both legs REQUIRED and AND-verified against a pinned `LogKey` (v0.2 §9.2). `name` is non-empty printable ASCII (`0x21`–`0x7e`, no `+`), naming the log key. |
...
| `checkpoint` / `prior_checkpoint` | not personal | C2SP signed-note text; see §2.12. |
...
### 2.16 Deliberately never recorded

No artifact defined by `attest-v0.1.md` or `attest-v0.2.md` has a field for any of the following. This is a design decision, visible in the field registries above and in the schema, not an operational recommendation:

- **No plaintext buyer identity.** The schema lists exactly three properties for `buyer` — `commitment`, `identifier_type`, `pubkey` — and none of them is a name, an account name, or a customer number.
- **No email address.** `email` appears only as an `identifier_type` value naming which population an opaque commitment was computed over; the address itself is never written anywhere.
- **No payment data.** No price, no currency, no total, no payment method, no card, account, or transaction reference. v0.1 §2 places payment processing out of scope outright: a receipt records the outcome of a purchase, not the purchase transaction, and MUST NOT be construed as a payment instrument.
- **No device identifiers.** No hardware identifier, no installation identifier, no machine fingerprint, in any of the document types inventoried above.
- **No IP addresses.** No artifact records one. attest defines document formats, canonicalization, and verification, and no delivery transport at all (v0.1 §2, TM-13), so whatever addresses a deployment's chosen channel exposes are exposed by that channel and recorded in nothing attest defines.
- **No postal or billing address, no date of birth or age, no jurisdiction of residence, and no special-category data.** `jurisdiction_flags` is an assertion about a sale's conditions, never about where the `buyer` is (§2.5).
- **No behavioural or usage data.** attest is content-free by design and a conforming implementation MUST NOT host or index the works a receipt refers to (v0.1 §2). Nothing in any artifact records that a work was opened, played, read, or downloaded.

Instead of an identity, the payload carries the buyer commitment described in §2.3: **scrypt over a store-scoped identifier by default, salted per receipt.** `issuer-account` — an identifier meaningful only within one store, whose disclosure links nothing globally — is the RECOMMENDED `identifier_type`, and the 16-raw-byte salt is generated fresh for every receipt, so the same identifier commits to an unrelated value in each one (v0.1 §8.1).

**The honest bound on this list.** It describes the field set the specifications define, not a constraint conforming implementations enforce. Because no payload object sets `additionalProperties: false` (v0.1 §5), a deploying `issuer` can place any of the above in an additional signed member and the receipt will verify cleanly — with a warning if that member is top-level, and with no mandated warning at all if it is nested inside `issuer`, `buyer`, `work`, `license`, or `survivability` (v0.1 §11.2, TM-21). The same holds without any extension at all for the open string fields in §2.4 and §2.5, which accept whatever an issuer writes into them. "Never recorded" is therefore a property of the specified format that a deployment inherits by default and can discard by choice, and no `verifier` can detect that it has been discarded.
...
- **Identification.** A `verifier` running the algorithm in v0.1 §11 without a disclosure never sees an identifier. `binding` stays `not_checked`, and `binding` is not a component of `ok` (v0.1 §11.1), so a receipt verifies fully green without the verifier learning anything about who holds it. Verification and identification are separable in this protocol, and by default they are separate.
...
A Stage 2 log sees only what §2.11 inventories: **manifest hashes and receipt-core hashes, never payloads.** Exactly two entry types are defined, each closed to exactly its required members, with every member a domain name, a version integer, or a lowercase-hex hash. No receipt, no field of a receipt, and no fragment of one is ever submitted to a log, and an entry whose type or member set does not match is rejected outright rather than partially trusted (v0.2 §8).
...
- **Identification.** Nothing in the mirrored files identifies anyone (§3.3). What a mirror sees of a requester's own network identity comes from the transport it chose to serve over, which attest does not define (v0.1 §2, TM-13).
...
- **Identification.** Not reachable from a shareable bundle alone. No salt is present, so a commitment cannot be tested against a candidate identifier even by dictionary attack; the identifiers the holder does gain are `receipt_id` values and whatever the `issuer` chose to write into the open string fields (§2.4). Where an `issuer` wrote an order or customer identifier into `work.identifiers`, or a per-buyer URI into `terms_uri`, the holder has a plaintext handle into that issuer's records and no attack was required to get it.
...
- **Correlation.** Confined to one receipt by design: this is what the per-receipt sharing primitive is for, and what per-receipt salts enforce — a salt disclosed here is useless against every other receipt the `buyer` holds (v0.1 §8.1, TM-19). The bound is real and it is also the whole of it: nothing prevents a recipient that accumulates disclosures for several receipts from joining them, and each additional disclosure extends the joinable set by one.
