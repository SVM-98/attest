...
| PC-18 | No conformance-corpus envelope carries a top-level `delivery` member. | corpus | No `envelope.json` or `envelope.raw.json` under `docs/spec/vectors/` contains a top-level `delivery` member. Separately, `salt_b64u` appears only in the four `disclosure.json` inputs named in PC-09. |
