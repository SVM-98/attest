...
A checkpoint is an aggregate statement about the whole tree. Its defined `tree_size`, `root`, signature key-hash, and signature legs carry no per-receipt or per-buyer value. `origin` and signature `name` are free strings within their respective printable-ASCII grammars, whose content the specifications do not constrain semantically.
...
- **Checkpoint free text.** A checkpoint's `origin` and its signature `name` carry no semantic constraint within their respective grammars: `origin` is non-empty ASCII `0x20`–`0x7e`, while `name` is non-empty ASCII `0x21`–`0x7e` and excludes `+` (v0.2 §9.3, §2.12). Either may carry a person's name, an email address, or another identifier. They are chosen by the `log operator`, describe the log rather than any `buyer` or `issuer`, and travel inside every evidence bundle that carries a checkpoint whole (§2.13).

The `issuer` hint is receipt content; the checkpoint strings are not. Neither residue is buyer content. The checkpoint strings are free-text or naming members chosen by a party operating infrastructure, and a deploying `log operator` controls those strings.
...
**For buyer data, the tension does not arise in the defined entry fields, because none is in the log.** No buyer identity or content is admitted to an entry (§5.1). A `receipt` entry does retain its non-authenticated `issuer` hint from the payload and an opaque `core_sha256` commitment, but no other payload field or plaintext fragment. The buyer's own receipt is the buyer's copy, held by the buyer and exportable as a portable file set (v0.1 §14.1); the `issuer`'s issuance records are the issuer's own and were never in the log either (§3.1). There is consequently no buyer content in the log for an erasure request to reach — not because deletion is refused, but because the data is not there. The one buyer-derived value that is present, a `receipt` entry's `core_sha256`, is an opaque hash whose preimage the log does not hold and cannot reconstruct (§5.2).
...
| `log operator` | Handles entries with no buyer fields and no receipt-payload value other than a `receipt` entry's non-authenticated `issuer` hint and opaque core commitment, plus the free text it writes itself. | §5.1, §3.3 |
...
- **The log holds no buyer fields to retain.** A `receipt` entry retains its non-authenticated `issuer` hint from the payload and an opaque core commitment, but no other payload field; the opaque hash's preimage cannot be reconstructed (§5.1, §5.3).
