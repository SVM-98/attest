...
**Governing scope rule.** Every statement anywhere in this document that an artifact "carries no buyer-derived value", "is not about a buyer", or uses an equivalent absence formulation is a statement about the members the specifications define and constrain. It is never a claim about what a deploying `issuer` or `log operator` may place in a free-form string, an open member, or a referenced document, because no conforming implementation can prevent or detect that content.
...
A key manifest is a public document, published by an `issuer` at a well-known URL. Its `kid` labels have no semantic-content enforcement; the field-by-field classifications below, rather than an absence guarantee for the whole artifact, state what its specified members may carry.
...
Log entries are **content-free by construction** in a precise sense: their closed schemas carry no defined buyer field and no direct receipt-payload value other than a `receipt` entry's non-authenticated `issuer` hint and its opaque signed-receipt-core commitment. The reference verifier constructs that hint from `payload.issuer.id` when it computes the expected `receipt` entry (`src/attest/verify.py`); it does not make the hint authenticated. Exactly two versioned types are defined, each with exactly the required member set and no more; unknown members are rejected outright rather than silently tolerated; and every member is a domain name, a version integer, or a lowercase-hex hash. That closure leaves no defined member for a buyer field or any further direct receipt-payload content (TM-51). It admits no plaintext buyer content, while the `core_sha256` member remains the pseudonymous, buyer-derived per-receipt value classified below.
...
A checkpoint is an aggregate statement about the whole tree. In its defined member set, `tree_size`, `root`, signature key-hash, and signature legs are not designated per-receipt or buyer fields; `root` is an aggregate over leaf hashes. `origin` and signature `name` are free strings within their respective printable-ASCII grammars, whose content the specifications do not constrain semantically.
...
| `checkpoint` / `prior_checkpoint` | potentially personal, user-controlled | C2SP signed-note text carried whole, so it inherits the classification of its parts: `tree_size` and `root` are not personal, but the `origin` header line and the signature `name`, within their respective grammars in §2.12, may carry a person's name, email address, or other identifier. |
...
A Stage 2 log sees only what §2.11 inventories: closed entries containing a non-authenticated `issuer` hint plus manifest or receipt-core hashes. For a `receipt` entry, the hint is copied from `payload.issuer.id`; beyond that hint and the opaque core commitment, the defined entry shape carries no direct receipt-payload value and no buyer field. **Content-free** here has that bounded meaning; it does not mean an entry has no potentially personal data, because `issuer` is potentially personal about the `issuer` and `core_sha256` is pseudonymous. Exactly two entry types are defined, each closed to exactly its required members, with every member a domain name, a version integer, or a lowercase-hex hash. An entry whose type or member set does not match is rejected outright rather than partially trusted (v0.2 §8).
...
- **Disclosure.** The log discloses that an artifact with a given hash existed at a point in the log's history, and nothing about who was entitled to write it — `corroboration` says an artifact was independently observable, never who was entitled to write it, and the log NEVER upgrades `trust` (v0.2 §15 items 3 and 4, TM-26). Checkpoints disclose aggregate tree size and a Merkle root, neither attributable to anyone, plus whatever the `log operator` itself chose to put in the `origin` line and the signature `name` within their respective grammars — `origin` is non-empty ASCII `0x20`–`0x7e`, while `name` is non-empty ASCII `0x21`–`0x7e` and excludes `+`. Those grammars do not limit the strings to information about the operator or exclude buyer- or issuer-derived content (§2.12).

### 3.4 `mirror operator`

A `mirror operator` republishes the log's static, mirrorable file set — `entries.jsonl`, tiles, checkpoints (v0.2 §7.2). It therefore learns from the files exactly what §3.3 describes and no more: it holds the same closed entries, including non-authenticated `issuer` hints and opaque core commitments, but no defined buyer fields or further direct receipt-payload values. Everything a mirror serves is untrusted evidence, exactly like the log's own primary host or an adversary (v0.2 §10.2, TM-27) — a trust property, and it does not change what the mirror sees.
...
- **Identification.** The closed, defined entry shapes contain no direct buyer content, but their `issuer` hints can identify an `issuer` (§3.3, §2.11). What a mirror sees of a requester's own network identity comes from the transport it chose to serve over, which attest does not define (v0.1 §2, TM-13).
...
> **A defined log entry carries no buyer field and no direct receipt-payload value except a `receipt` entry's non-authenticated `issuer` hint and opaque `core_sha256` commitment to the signed receipt core.**

That is a claim about the defined entry members and specified receipt and buyer values, and it is exactly the claim the entry schemas support. For receipt evidence, the reference verifier constructs `issuer` from `payload.issuer.id`; `core_sha256` is the opaque commitment defined in v0.2 §12 (`src/attest/verify.py`, §2.11). Exactly two entry types are defined; each is closed to exactly its required member set; unknown members are rejected outright rather than tolerated; and every member is a type literal, a lowercase DNS name, a version integer, or a 64-character lowercase-hex hash (v0.2 §8, §2.11). No defined buyer field or further direct receipt-payload value has a place in that shape, which is what closes the entry-poisoning surface (TM-51).

**It is not a claim that a log entry carries no potentially personal data at all**, and this document does not make that claim. Two residues are stated in the inventory and are restated here as the bounded content the definition above calls out:

- **The `issuer` member.** Both entry types carry a DNS name, classified `potentially personal, user-controlled` because a sole trader's domain is personal data about that trader (§2.11, §2.2). In a `receipt` entry, the reference verifier copies it from `payload.issuer.id`, so it is the explicit receipt-payload exception to the definition above; it is also the only member that is not a hash, so an entry stream discloses claimed per-issuer activity in submission order (§3.3). It is normatively a NON-AUTHENTICATED hint (v0.2 §8), and neither specification defines submitter authentication, quotas, or rate limits — a tracked gap carried by TM-51 — so anyone may submit an entry naming any issuer and the resulting picture is unverifiable in both directions.
- **Checkpoint free text.** A checkpoint's `origin` and its signature `name` carry no semantic constraint within their respective grammars: `origin` is non-empty ASCII `0x20`–`0x7e`, while `name` is non-empty ASCII `0x21`–`0x7e` and excludes `+` (v0.2 §9.3, §2.12). Either may carry a person's name, an email address, or another identifier. They are chosen by the `log operator` and conventionally name the log or its key, but their grammars do not enforce that they exclude buyer- or issuer-derived content; they travel inside every evidence bundle that carries a checkpoint whole (§2.13).

The `issuer` hint is receipt-payload content; checkpoint strings are not receipt-payload members in the defined data flow. Neither checkpoint grammar enforces how its strings relate to a buyer or issuer. The `core_sha256` member is not plaintext, but it is a per-receipt buyer-derived pseudonymous value (§2.11); the checkpoint strings are free-text or naming members chosen by a party operating infrastructure, and a deploying `log operator` controls those strings.
...
**For buyer fields and plaintext buyer content, the tension does not arise in the defined entry fields: the log carries neither.** A `receipt` entry does retain its non-authenticated `issuer` hint from the payload and an opaque `core_sha256` commitment, but no other direct payload field or plaintext fragment. That does not make the log free of buyer-derived values: `core_sha256` is a per-receipt value derived from the signed core, which includes buyer members, and §2.11 classifies it as pseudonymous. Its privacy property is confirmation of a candidate receipt a holder already has, never recovery of an artifact the holder does not already hold (§5.2). The buyer's own receipt is the buyer's copy, held by the buyer and exportable as a portable file set (v0.1 §14.1); the `issuer`'s issuance records are the issuer's own and were never in the log either (§3.1). The append-only log therefore has no defined buyer field or plaintext buyer content for an erasure request to reach, while retaining that bounded pseudonymous per-receipt value.

**For the free-text and naming members of §5.1, the tension is real, bounded, and worth stating.** An `issuer` DNS name inside an entry cannot be removed or altered within the append-only `entries.jsonl`. Within an existing signed checkpoint copy, its `origin` and key `name` cannot be edited without invalidating verification; checkpoint strings may also persist in distributed evidence. The protocol retains no append-only checkpoint history: checkpoints are derived artifacts, while only `entries.jsonl` is append-only (v0.2 §7.2, §9.1–§9.3). What accumulates in the entries is a per-issuer activity record — how many entries claim a given issuer, and in what order — that cannot be withdrawn and, because submission is unauthenticated, cannot be attributed either. A deploying `log operator` controls what it writes into `origin` and `name`; nothing in the protocol constrains what a submitter writes into `issuer`.
...
| `log operator` | Handles entries with no defined buyer fields and no direct receipt-payload value other than a `receipt` entry's non-authenticated `issuer` hint and opaque core commitment, plus the free text it writes itself; the core commitment is a pseudonymous per-receipt value. | §5.1, §3.3 |
...
- **The defined log entry holds no buyer field to retain.** A `receipt` entry retains its non-authenticated `issuer` hint from the payload and an opaque, pseudonymous core commitment, but no other direct payload field; the hash's preimage cannot be reconstructed (§5.1, §5.3).
...
| PC-03 | A closed `receipt` log entry has no defined buyer field and no direct receipt-payload value other than its non-authenticated `issuer` hint; `core_sha256` is a pseudonymous commitment to the signed receipt core. | spec-text | v0.2 §8 defines exactly two closed entry types — `key-manifest` (`type`, `issuer`, `manifest_version`, `manifest_sha256`) and `receipt` (`type`, `issuer`, `core_sha256`) — with any other type or member set rejected outright. For receipt evidence, `src/attest/verify.py` constructs the expected entry with `issuer` from `payload.issuer.id` and `core_sha256` from the v0.2 §12 signed-receipt-core hash; it supplies no defined buyer field or other direct payload value, while the core hash remains derived from the signed core. |
