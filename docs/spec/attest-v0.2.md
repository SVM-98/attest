...
- **Status**: Normative, v0.2 (Stage 1 AND Stage 2 — see §1 Scope; Stage 2b witness federation remains forthcoming, §16)
- **Date**: 2026-07-18
...
- **`entries.jsonl` is the SOLE source of truth.** One JSON entry object per line, append-only. Every other on-disk artifact (tiles, the checkpoint candidate, the signed checkpoint) is a derived cache, rebuildable from `entries.jsonl` alone by re-running RFC 6962 `MTH` (v0.1's Merkle build, `tlog.build_tree`) over the re-encoded entries.
...
A conforming implementation MUST rebuild its tile cache from `entries.jsonl` on every append (never patch a tile incrementally) and MUST treat a tile/entries mismatch as a fatal integrity failure rather than trusting the tile.
...
Pinned log keys (§9.2 `LogKey`) ship baked into the verifier's own trust store, distributed and rotated out-of-band from any bundle. A `proofs/log-keys.json` copy that MAY travel inside a bundle (§14) is UNTRUSTED metadata: a conforming verifier MUST byte-match it against an already-pinned `LogKey` or ignore it outright — it is never itself a trust root.

## 8. Log entry schemas

Every entry admitted to the log is a CLOSED, versioned, JCS-canonicalized object (`tlog.encode_entry`): unknown members are rejected outright (no silent extension of a schema in production use — schema extension is a registry-governed change, out of this document's scope), and the canonical bytes produced are exactly what gets RFC 6962 leaf-hashed (`tlog.leaf_hash(SHA-256(0x00 || entry_bytes))`) into the log. Exactly two entry types are defined:
...
For the same reason, every diagnostic that renders an untrusted origin/name/tree-size/signature-line value MUST do so with **Python `ascii()` semantics on both cores**: printable ASCII passes through unchanged, and every other code point renders as `\xNN` (one byte, `< 0x100`), `\uNNNN` (BMP), or `\UNNNNNNNN` (astral), matching CPython's `ascii()` builtin exactly — never each runtime's own native string-escaping convention. The reference implementation is `tlog.py`'s `_trunc_repr` (Python, calling `ascii()` directly) and `verifiers/ts/src/messages.ts`'s `pyStage2StringRepr` (TypeScript, a faithful reimplementation) — the two MUST render byte-identical diagnostics for the same hostile input.
...
`AnchorVerdict.anchored_before` is the MINIMUM pinned header time across every verified `ots` proof in the bundle (never a single timestamping authority's self-asserted `genTime`) — a conservative lower bound, not an upper one. A conforming `AnchorPolicy` requires ≥1 verified `ots` proof for `anchored_before:<T>` to appear at all; `AnchorPolicy` MAY require more (a quorum) as local trust-store policy, but this document's reference implementation accepts a single verified `ots` proof as sufficient.
...
**Fix, normative for every Stage-2-and-later implementation.** Revocation records now share the same hybrid AND-rule discipline that key manifests (v0.2 §2.3) and artifact manifests (v0.1 §7.2) already had: if the signing key's own manifest entry carries `pub_ml_dsa_65` (i.e. the signer is hybrid), the revocation record's signature block MUST also carry a valid `sig_ml_dsa_65` leg over the same signed bytes, or the record is treated as invalid and ignored — a downgraded, Ed25519-only signature against a hybrid key's authority is never honored, regardless of how early or late it is presented. An Ed25519-only signer's revocation record carrying a stray `sig_ml_dsa_65` leg likewise fails closed (a record cannot claim hybrid protection for a key that never had a PQ public key). This is symmetric, fail-closed AND-verification in both directions, exactly mirroring §2.3's manifest-signature rule.
...
**The ONLY accepted member shape is `proofs/<ULID>.json`**, where `<ULID>` is exactly the 26-character Crockford base32 ULID the receipt schema already pins `receipt_id` to (first character in `0`–`7`, matching the schema's own timestamp-prefix constraint). A conforming importer MUST reject any other shape under `proofs/` — a nested path, a nested member, a nonexistent-`.json` suffix, or a filename that is not itself a syntactically valid ULID — BEFORE deriving any filesystem path from it: the member name is attacker-supplied bundle content, and letting an unvalidated shape reach a filesystem join is exactly the traversal hazard the ULID-only grammar exists to close. A conforming exporter, symmetrically, MUST derive the on-disk proof filename ONLY from an already schema-valid `receipt_id` belonging to a receipt actually present in the same export — an evidence bundle keyed to a receipt_id NOT among the receipts being exported is silently dropped (it would be orphaned evidence for a receipt the recipient never receives).
...
| `28e` | A verifying prior (smaller) checkpoint plus a genuine RFC 6962 consistency proof against the current checkpoint → still just `"logged"` — consistency rules out equivocation, it does not itself upgrade standing. |
...
| `28k` | An `rfc3161`-only anchor proof → opaque classical corroboration only, `transparency` stays `"logged"`, never `anchored_before:<T>`; the verbatim RFC 3161 warning literal is asserted (§11.1). **Documented adaptation**: no leaf here sets a `crqc_horizon` — an rfc3161-only proof never sets `pq_surviving` and therefore never reaches `anchor.passes_horizon` regardless of horizon configuration, so a configured horizon would add configuration surface, not additional test coverage, for this specific property. |
...
**Non-normative note:** group-28 vectors are generated deterministically by [`tools/gen_vectors.py`](../../tools/gen_vectors.py)'s `gen_28_transparency`, the same generator and determinism gate as every other group. Checkpoint/log fixtures use fixed keys and seeds; the `ots`/`rfc3161` anchor fixtures are frozen and committed (a committed OTS proof plus a pinned test Bitcoin header; a committed RFC 3161 TimeStampToken plus a test TSA chain) — no network access occurs in any conformance test, ever.
