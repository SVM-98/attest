...
- **Verdict:** Mitigated — v0.1 §14.1, v0.1 §14.2, v0.1 §13.  The file a buyer is invited to share carries no *defined* salt or buyer-key member — export strips `delivery.salt` from every envelope, and the bound is on defined members rather than on content, as TM-13's residual records; the private file is separately named and documented as unshareable; the bundle's generated `README.html` MUST explain which file MUST NOT be shared; and per-receipt `disclose` exists so that sharing one receipt never means forwarding a library.
