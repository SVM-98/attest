...
- **Verdict:** Mitigated — v0.2 §8, v0.2 §7.2.  Entries are closed by construction — a closure of *shape*, not of content: exactly three versioned entry types are defined (`key-manifest`, `receipt`, and `revocation-record`, the last added by rev 5), each with exactly the required member set and no more, unknown members are rejected outright rather than silently tolerated, and every member is a domain name, a version integer, or a lowercase-hex field. That closes the vectors this entry names: a schema extension cannot be smuggled in for a consumer to act on, and attribution cannot be manufactured, because the `issuer` member of a `receipt` or `revocation-record` entry is normatively a NON-AUTHENTICATED hint for log browsing that a conforming verifier MUST NOT read as attribution, while an entry whose type or member set does not match resolves to `transparency: "not_checked"` rather than being partially trusted. `entries.jsonl` is the sole source of truth and the tile cache carries no authority, so a poisoned cache cannot outlive a rebuild. The separate admission-volume slice is a tracked protocol gap (§6.3).
...
- **Double assignment.** The revision MUST define what a verifier reports when two records transfer the same receipt to different parties. Detecting that condition requires an ordering source the current documents do not provide for transfer records specifically — no entry type for them exists, the same absence that still applies to artifact manifests in full (v0.2 §13.1). Revocation records are no longer a clean example of this same gap: `revocation-record` is now a loggable entry type since rev 5 (v0.2 §8), though logging any individual record remains at the issuer's discretion rather than required, except for the narrow `refund_window` deadline rule (§15 item 5). Extending the log's closed entry schema to transfer records, or defining an equivalent ordering mechanism, is a precondition rather than an optimization.
