...
- **Verdict:** Out of scope — v0.1 §7.3, v0.1 §12.1, v0.1 §12.2. The specifications authenticate `revoked_at` and key status but do not establish their truthfulness or distinguish coercion from voluntary signing. A signed refund-window time can be backdated inside the key-validity window, policy-class records are honored as signed, and a compelled `compromised` marking invalidates every signature under that key. Signer intent and compulsion are scope boundaries (§7) — no signature scheme distinguishes a compelled signer from a willing one — but deadline-sensitive revocation effectiveness is a tracked protocol gap (§6.3): revocation records are not loggable entry types (v0.2 §8, v0.2 §15 item 5), so no verifier rule can require proof that a record existed by the relevant deadline.
...
| OpenTimestamps pre-anchoring — the anchor commits to `checkpoint.note_bytes` while the signature lines are excluded (v0.2 §9.1, §11.1), so a chosen unsigned note can be pre-anchored and signed later by a holder of both log keys | TM-33 | Anchor coverage extended over the signed checkpoint, including its signature lines |
| Deadline-unenforceable revocation effectiveness — revocation records are not loggable entry types (v0.2 §8, v0.2 §15 item 5), so no rule can require proof that a record existed by a relevant deadline; ordinary logging cannot prove a record's asserted creation time, because `anchored_before` is only an upper bound on existence (v0.2 §11.1) | TM-47 | A log entry committing to the signed revocation record, plus an explicit verifier rule that makes a deadline-sensitive revocation effective only when that entry was logged and anchored no later than the relevant deadline |
...
The table records only the mechanistically closable slices. TM-12's row concerns stripping before acknowledged receipt; once neither the `issuer` nor the `buyer` retains the salt, recovery is impossible and that post-custody loss remains a §7 boundary. TM-47 appears only for deadline-sensitive effectiveness; signer intent and compulsion remain §7 boundaries because no signature scheme distinguishes a compelled signer from a willing one. TM-49 belongs here despite v0.2 §15 item 1 stating the limitation normatively: §6.2 identifies witness federation as the missing mechanism, so it is a tracked omission rather than a deliberate exclusion.
...
| Signer intent and coercion | A valid signature establishes what its holder signed, never whether that holder acted voluntarily; neither specification can distinguish a compelled revocation or key-status publication from a willing one (TM-47). |
